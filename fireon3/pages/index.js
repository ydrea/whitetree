import Head from "next/head";
import Image from "next/image";
// import Link from "next/link";

import Login from "../components/Login";
import Account from "../components/Account";
import { useAuth } from "../context/authContext";
import styles from "../styles/Home.module.css";
// import { replacer } from "../hooks/regEx";
import { useEffect, useState, useRef } from "react";
import { Mod } from "../components/Modal";
import Form from "../components/Form";
import FormLarge from "../components/FormLarge";
//
import { doc, setDoc, deleteField } from "firebase/firestore";
import { db, user } from "../db/firestore";
import useFetchGet from "../hooks/fetchGET";
import FormImage from "../components/FormImage";

export default function Home() {
  const { currentUser } = useAuth();
  console.log("====================================");
  console.log(currentUser);
  console.log("====================================");
  // const uid = "RVEHQH4IhWNQYJeAiPbWk9uoyQo1";

  const [openModal, openModalSet] = useState(false);
  //
  const [cementAdd, cementAddSet] = useState(false);
  //
  const [cement, cementSet] = useState("");
  const [cementList, cementListSet] = useState({});

  //prettier-ignore

  async function handleAddCement() {
    if (!cement) {
      return;
    }
    //Object.keys() returns an array
    const newKey =
      // 1, ili...
      Object.keys(cementList).length === 0
        ? 1
        : //...najveci do sada +1
          Math.max(...Object.keys(cementList)) + 1;
    //se dodaje na listu
    cementListSet({ ...cementList, [newKey]: cement });
    //u bazu kod usera

    const userRef = doc(db, "users", currentUser.uid);
    console.log("REF", userRef);

    //prettier-ignore
    await setDoc(
      userRef,
      {
        "cement": { [newKey]: cement },
      },
      { merge: true }
    );
    cementSet("");
  }
  //
  function replacer(email) {
    const newEmail = email.replace(/\@.*/, "... ");
    console.log("new", newEmail);
    return newEmail;
  }

  return (
    <div className={styles.container}>
      <Head>
        <title>white tree</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <button onClick={() => openModalSet(true)}>open</button>
      <Mod open={openModal} closeMe={() => openModalSet(false)}>
        dalMo
      </Mod>

      <main className={styles.main}>
        {!currentUser && <Login />}
        {currentUser && <Account />}

        <div className={styles.grid}>
          <div className={styles.card}>
            <FormImage />
          </div>

          <div className="flex flex-col w-full max-w-{65ch} p-3 items-stretch">
            {" "}
            <div>
              <FormLarge
                cement={cement}
                cementSet={cementSet}
                cementList={cementList}
                cementListSet={cementListSet}
                handleAddCement={handleAddCement}
              />
            </div>
          </div>

          {Object.keys(cementList).map((cement, index) => (
            <div
              className="flex flex-col w-full max-w-auto gap-3  mt-7 p-3
              border-transparent border rounded border-1
              duration-300 hover:border-olive"
              key={index}
            >
              <h5 className="flex flex-1 flex-row justify-between  w-full max-w-auto ">
                {" "}
                {replacer(currentUser.email)}
                <div className="flex-2 px-12">
                  <i
                    className="fa-solid fa-pencil mx-7
              duration-200 hover:rotate-90 hover:scale-125 cursor-pointer"
                  ></i>
                  <i class="fa-solid fa-trash-can hover:scale-150 cursor-pointer"></i>
                </div>
              </h5>
              <p className=" border-t-2 border-olive border-opacity-40">
                {cementList[cement]}
              </p>
            </div>
          ))}
        </div>
      </main>

      <footer className={styles.footer}>
        <a
          href="https://vercel.com?utm_source=create-next-app&utm_medium=default-template&utm_campaign=create-next-app"
          target="_blank"
          rel="noopener noreferrer"
        >
          Powered by{" "}
          <span className={styles.logo}>
            <Image src="/vercel.svg" alt="Vercel Logo" width={72} height={16} />
          </span>
        </a>
      </footer>
    </div>
  );
}
